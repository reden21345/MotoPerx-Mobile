...
const FuelGame = ({ navigation }) => {
  ...
  useEffect(() => {
    if (gameState === "challenge" && fuelChallenge) {
      ...
      const scenario = fuelChallenge.challenge || fuelChallenge.scenario || fuelChallenge;
      if (scenario && scenario.initialFuel) {
  ...
  const startChallenge = async (competitive = false) => {
    ...
    try {
      const result = await dispatch(getFuelChallenge({ isCompetitive: competitive })).unwrap();
      // Handle both response formats: { challenge: ... } or { scenario: ... }
      const scenario = result.challenge || result.scenario;
      if (scenario) {
        setCurrentFuel(scenario.initialFuel);
      }
    } catch (error) {
      Alert.alert("Error", error || "Failed to load challenge. Please try again.");
      setGameState("menu");
    }
  };
  ...
  const finishChallenge = async (finalDecisions, scenario) => {
    ...
    try {
      ...
      Alert.alert("Error", error || "Failed to submit challenge. Please try again.");
    }
  };
    ...
    <ScrollView contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.modeContainer}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <View style={[styles.modeCardContent, competitiveAttempts >= 2 && styles.disabledCard]}>
        ...
        >
          <View style={styles.modeCardContent}>
            <View style={[styles.modeIcon, { backgroundColor: "rgba(59, 130, 246, 0.2)" }]}>
            ...
            <View style={styles.modeInfo}>
              ...
              <View style={styles.modeBadges}>
                <View style={[styles.modeBadge, { backgroundColor: "rgba(59, 130, 246, 0.3)" }]}>
        ...
        <View style={styles.rankCard}>
          ...
          <View style={styles.rankContent}>
            ...
            <View style={styles.rankItem}>
              ...
              <Text style={styles.rankValue}>{userBestScore.bestScore?.toFixed(0)}</Text>
            </View>
            <View style={styles.rankDivider} />
            <View style={styles.rankItem}>
              <Text style={styles.rankLabel}>Fuel Left</Text>
              <Text style={styles.rankValue}>{userBestScore.fuelRemaining?.toFixed(1)}L</Text>
            </View>
          </View>
        </View>
      )}
  ...
  const renderChallenge = () => {
    ...
    return (
      <View style={styles.challengeContainer}>
          ...
          <View style={styles.scenarioCard}>
            ...
            <View style={styles.scenarioStats}>
              <View style={styles.scenarioStat}>
                ...
                <Text style={styles.scenarioStatText}>{scenario.distance} km</Text>
              </View>
              <View style={styles.scenarioStat}>
              ...
              {scenario.terrain && (
                <View style={styles.scenarioStat}>
                  ...
                  <Text style={styles.scenarioStatText}>{scenario.terrain}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {/* Fuel Gauge */}
        <Animated.View style={[styles.fuelGauge, { transform: [{ scale: fuelAnim }] }]}>
              ...
              <View
                style={[
                  styles.fuelBar,
                  {
                    ...
                    backgroundColor: getFuelColor(currentFuel, scenario.initialFuel),
                  },
                ]}
              />
            </View>
          ...
          {fuelPercentage < 30 && (
            <Text style={styles.fuelWarning}>⚠️ Low fuel! Choose carefully</Text>
          )}
        </Animated.View>
            ...
            {currentQuestion.options.map((option, index) => (
              ...
              >
                <View style={styles.choiceContent}>
                  ...
                  <View style={styles.fuelCostBadge}>
                    <Ionicons name="water" size={14} color="#FFF" />
                    <Text style={styles.fuelCostText}>-{option.fuelCost.toFixed(1)}L</Text>
                  </View>
                </View>
              </TouchableOpacity>
            ))}
    ...
    return (
      <ScrollView contentContainerStyle={styles.resultContainer}>
        <View style={styles.resultHeader}>
          <Ionicons
            name={efficiency >= 70 ? "trophy" : efficiency >= 50 ? "ribbon" : "medal"}
            size={80}
            color={efficiency >= 70 ? "#FFD700" : efficiency >= 50 ? "#C0C0C0" : "#CD7F32"}
          />
          <Text style={styles.resultTitle}>Challenge Complete!</Text>
          <Text style={styles.resultScore}>{fuelResult.efficiencyScore.toFixed(0)}%</Text>
        ...
        <View style={styles.resultStats}>
          <View style={styles.resultStatBox}>
            <Text style={styles.resultStatValue}>{fuelResult.correctDecisions}</Text>
            <Text style={styles.resultStatLabel}>Optimal Choices</Text>
          </View>
          <View style={styles.resultStatBox}>
            <Text style={styles.resultStatValue}>{fuelResult.totalQuestions}</Text>
            <Text style={styles.resultStatLabel}>Total Decisions</Text>
          </View>
          <View style={styles.resultStatBox}>
            <Text style={styles.resultStatValue}>
              {((fuelResult.correctDecisions / fuelResult.totalQuestions) * 100).toFixed(0)}%
            </Text>
        ...
        {isCompetitive && fuelResult.ranking && (
          <View style={styles.rankingCard}>
            ...
            <Text style={styles.rankingSubtext}>on the weekly leaderboard!</Text>
          </View>
        )}
              ...
              fuelResult.decisions
                .map(
                  (d, i) =>
                    ...
                    `Fuel: -${d.fuelCost.toFixed(1)}L ${d.isOptimal ? "✅" : "❌"}\n` +
                    `${d.feedback}`
                )
                .join("\n\n")
            );
          ...
          <View key={index} style={styles.historyItem}>
            <View style={styles.historyLeft}>
              ...
              <View style={styles.historyInfo}>
                <Text style={styles.historyTitle}>{challenge.scenarioTitle}</Text>
            ...
            <View style={styles.historyRight}>
              <Text style={styles.historyScore}>{challenge.efficiencyScore.toFixed(0)}%</Text>
              <Text style={styles.historyFuel}>{challenge.fuelRemaining.toFixed(1)}L left</Text>
          ...
          <View key={entry.rank} style={styles.leaderboardItem}>
            ...
            <View style={styles.leaderboardInfo}>
              ...
              <Text style={styles.leaderboardDetails}>
                {entry.fuelRemaining?.toFixed(1)}L remaining • {entry.correctDecisions} optimal
              </Text>
            </View>
            <Text style={styles.leaderboardScore}>{entry.bestScore.toFixed(0)}%</Text>
          </View>
        ))
      ) : (
